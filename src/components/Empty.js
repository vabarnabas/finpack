...
const Empty = (props) => {
    ...
    return (
        <div className='relative w-full h-full flex items-center justify-center group bg-slate-200 hover:bg-slate-300 rounded-lg'>
            <div className={`hidden group-hover:grid grid-flow-col ${'grid-cols-' + Math.min(4,menuItems.length)} gap-4 mx-10 bg-white rounded-3xl px-6 py-3 shadow shadow-slate-400 text-slate-300`}>
                {menuItems.map((item) => (
